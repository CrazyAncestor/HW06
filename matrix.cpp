...
void matrix::Error_Message(const matrix &b){
    double sum = 0;
    double ave = 0;
    for(int i=0;i<dim;i++){
        for(int j=0;j<dim;j++){
    ...
    cout<<(double)(sum/dim/dim)<<endl;
}
double matrix::Error(const matrix& b) {
    double sum = 0;
    double ave = 0;
    for (int i = 0; i < dim; i++) {
        for (int j = 0; j < dim; j++) {
...
void matrix::SOR_smoothing(const matrix& rho, const matrix& ans, double omega, double  err_threshold, int num_thread) {
    double err = 1.0;
    int step = 0;
